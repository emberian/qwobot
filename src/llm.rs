use crate::error::{QwobotError, Result};
...
fn resolve_model_path(model_id: &str, quantization: &str) -> Result<PathBuf> {
    ...
    Err(QwobotError::ModelLoad(format!(
...
async fn resolve_model_path_with_progress(
    ...
    .await
    .map_err(|e| QwobotError::ModelLoad(format!("Task join error: {e}")))?
}
    ...
    pub async fn new(
        ...
        let inner = tokio::task::spawn_blocking(move || {
            let backend = LlamaBackend::init()
                .map_err(|e| QwobotError::ModelLoad(format!("Backend init failed: {e}")))?;
            ...
            let model = LlamaModel::load_from_file(&backend, &path, &model_params)
                .map_err(|e| QwobotError::ModelLoad(format!("Model load failed: {e}")))?;
            ...
            Ok::<_, QwobotError>(LlmInner { backend, model })
        })
        .await
        .map_err(|e| QwobotError::ModelLoad(format!("Task join error: {e}")))??;
    ...
    pub async fn switch_model(
        ...
        let load_result = tokio::task::spawn_blocking(move || {
            ...
            let mut inner_guard = inner.lock()
                .map_err(|e| QwobotError::ModelLoad(format!("Lock error: {e}")))?;
    ...
    pub async fn generate(&self, prompt: &str) -> Result<String> {
        ...
        let output = tokio::task::spawn_blocking(move || {
            let inner = inner.lock().map_err(|e| {
                QwobotError::Inference(format!("Failed to acquire lock: {e}"))
            })?;
            ...
            let formatted_prompt = if let Ok(template) = inner.model.chat_template(None) {
                let messages = vec![LlamaChatMessage::new("user".to_string(), prompt.clone())
                    .map_err(|e| QwobotError::Inference(format!("Message creation failed: {e}")))?];

                inner
                    .model
                    .apply_chat_template(&template, &messages, true)
                    .map_err(|e| QwobotError::Inference(format!("Template apply failed: {e}")))?
            } else {
                prompt.clone()
            };
            ...
            let mut ctx = inner
                ...
                .map_err(|e| QwobotError::Inference(format!("Context creation failed: {e}")))?;

            // Tokenize
            let tokens = inner
                .model
                .str_to_token(&formatted_prompt, AddBos::Always)
                .map_err(|e| QwobotError::Inference(format!("Tokenization failed: {e}")))?;
            ...
            for (i, token) in tokens.iter().enumerate() {
                ...
                batch
                    ...
                    .map_err(|e| QwobotError::Inference(format!("Batch add failed: {e}")))?;
            }

            // Process prompt
            ctx.decode(&mut batch)
                .map_err(|e| QwobotError::Inference(format!("Prompt decode failed: {e}")))?;
            ...
            for _ in 0..max_tokens {
                ...
                ctx.decode(&mut batch)
                    .map_err(|e| QwobotError::Inference(format!("Decode failed: {e}")))?;
            ...
            Ok::<_, QwobotError>(output)
        })
        .await
        .map_err(|e| QwobotError::Inference(format!("Task join error: {e}")))??;
